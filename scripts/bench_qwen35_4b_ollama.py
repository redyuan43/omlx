...
"""Benchmark the local Ollama Qwen3.5-4B stack.

This is a standalone Ollama-native version of the existing Qwen3.5-4B
benchmarking scripts. It does not depend on the original control plane or
runtime services.
"""
...
def _restart_ollama_service(timeout_sec: int = 120) -> Dict[str, Any]:
    started = time.perf_counter()
...
def _run_single_session_recovery(
    *,
    ...
    turn3_messages = turn2_messages + [
        ...
        {
            "role": "user",
            "content": "After the service restart, answer with one different word.",
        },
    ]
    turn3 = _run_chat(
        base_url=base_url,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark the local Ollama Qwen3.5-4B stack")
    ...
    prefix_unit = "Ollama Qwen3.5 cache benchmark"
    if args.prefix_salt:
